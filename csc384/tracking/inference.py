...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        # check whether noisyDistance is None or not
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        if noisyDistance != None:
            weight_pos = util.Counter()
            for p in self.particles:
                dis = util.manhattanDistance(p,pacmanPosition)
                weight_pos[p] += emissionModel[dis]
            if weight_pos.totalCount() > 0:
                list = []
                for i in range(0, self.numParticles):
                    list.append(util.sample(weight_pos))
                self.particles = list
            else:
                self.initializeUniformly(gameState)
        # a ghost is captured by Pacman
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            ghost = self.setGhostPosition(gameState, p)
            # obtain distribution of new positions of ghost
            dist = self.getPositionDistribution(ghost)
            list.append(util.sample(dist))
        self.particles = list
